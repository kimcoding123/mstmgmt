...
$(document).ready(function(){
	...
	$.ajax({
		 url :"<c:url value='/srvcr/selectHtmlTags.ajax'/>"  // SrvController 에 정의
       ,type: "POST"
	    //,contentType:'application/json; charset=UTF-8'
       //,data :{'taskDivSclsCd':taskDivSclsCd}  
       ,data :{'scrId':scrId}  
       ,dataType: 'json'  	   
       ,async: false  // 엘리먼트가 먼저 구성되어야,  이후 함수들 처리 가능
       ,success : function(data){
       	//실제로는 ajax로 데이터 땡겨옴.
       	//setSelectSrvcrOptions(data.list);
       	setHtmlTagsNew(data.list);
       	
		}
	    ,error: function(){
			console.log("태그출력 ajax 에러");
	    	
	    }
	});
	
	//셋팅
	function setHtmlTags(list){
		$(list).each(function(){
	...
	//셋팅
	var allTags = "<article>시작</article>";
	
	function setHtmlTagsNew(list){
		$(list).each(function(){
			allTags = allTags + this.tags; 
		});
		console.log(allTags);
		$('#tags').append(allTags);
	...
	//$( "#import" ).load( "./css/style2.css" );
	//$(document).createStyleSheet( "/css/style_dyn.css" );
	
	//$(".subject-box-txt").css( {"font-size": "inherit"});
	//$('div .tit-txt').css( {
    //    "font-size": "24px",
    //    "line-height": "42px",
	...
	console.log("CustomTextEditor 실행전");
	...
	console.log("gridSrvcrItmValDtls 실행전");
	...
	console.log("gridSrvcrItmValDtlsHist 실행전");
	...
	console.log("setSelectSrvcrOptions 실행전");
	...
	console.log("searchSrvcrDtls 실행전");
